...
export const IGoogleAnalytics = DI.createInterface<IGoogleAnalytics>(
    "IGoogleAnalytics",
    (x) => x.singleton(Analytics)
);
...
export class Analytics {
        ...
        this.eventAggregator.subscribe(
            ...
            (payload: any) => {
                const activeComponent =
                    payload?.navigation?.scope?.router?.activeComponents[0];

                if (
                    // Ignore page fragments
                    this._options.pageTracking.ignore.fragments.some(
                        (fragment) =>
                            payload.navigation.fragment.includes(fragment)
                    ) ||
                    // Ignore routes
                    this._options.pageTracking.ignore.routes.some(
                        (route) => payload.navigation.instruction === route
                    ) ||
                    // Ignore route names
                    this._options.pageTracking.ignore.routeNames.some(
                        (routeName) =>
                            activeComponent.component.name === routeName
                    )
                )
                    return;
        ...
        window.onerror = function (
            ...
            if (typeof ga === "function") {
                ...
                if (options.exceptionTracking.applicationName != undefined) {
                    // @ts-ignore
                    exOptions.appName =
                        options.exceptionTracking.applicationName;
                }
                if (options.exceptionTracking.applicationVersion != undefined) {
                    // @ts-ignore
                    exOptions.appVersion =
                        options.exceptionTracking.applicationVersion;
                }
            ...
            if (typeof existingWindowErrorCallback === "function") {
                return existingWindowErrorCallback(
                    errorMessage,
                    url,
                    lineNumber,
                    columnNumber,
    ...
    _trackClick(evt) {
        ...
        this._log(
            "debug",
            `click: category '${tracking.category}', action '${tracking.action}', label '${tracking.label}', value '${tracking.value}'`
        );
    ...
    _trackPage(path, title) {
        ...
        if (!this._initialized) {
            this._log(
                "warn",
                "Try calling 'init()' before calling 'attach()'."
            );
            return;
        }
