import { bindable, BindingMode, IEventAggregator, INode } from 'aurelia';
...
export class PhotoStripCustomElement {
    ...
    async ready(what) {
        ...
        if (this.id != this.prev_id || this.restart) {
            ...
            this.source.then((result) => {
                const slides = result.photo_list;
            ...
            const n = this.settings.slide_show;
            if (n && !this.slideShow) {
    ...
    async start_slide_show(slides) {
        const n = this.calculate_covering_count(slides);
    ...
    drag_photos(event) {
        ...
        const { dx, dy, target } = event.detail;
        if (Math.abs(dy) > 10 * Math.abs(dx)) {
            ...
            const r = this.height / h;
            const parent = target.parentElement;
            const m = parent.clientWidth / 2;
    ...
    dispatch_height_change() {
        const changeEvent = new CustomEvent('height_change', {
            detail: {
    ...
    auto_next_slide() {
        const target = this.slideList;
        const dir = getComputedStyle(target).direction === 'ltr' ? -1 : +1;
    ...
    calculate_widths() {
        for (const slide of this.slides) {
            ...
            const r = this.height / slide.height;
            const w = Math.round(r * slide.width);
            const img = document.getElementById('img-' + slide.photo_id);
            if (img) {
    ...
    calculate_covering_count(slides) {
        const container_width = this.theme.width;
        let width = 0;
        let n = 0;
        for (const slide of slides) {
            n += 1;
    ...
    on_click(event, slide) {
        ...
        if (this.action_key) {
            const offset = this.shift_photos(0);
    ...
    goto_photo_table() {
        const photo_ids = this.slides.map((slide) => slide.photo_id);
    ...
    get dummy() {
        if (!this.theme.is_desktop) {
            const height = Math.round(this.theme.height / 3);
