...
export class GoogleMaps implements ICustomElementViewModel {
    ...
    constructor(
        ...
        @IConfigure readonly config: IConfigure,
        ...
        return this._mapPromise.then(() => {
            ...
            this.createMarker({
                ...
                if (marker.infoWindow) {
                    createdMarker.infoWindow = new (<any>(
                        window
    ...
    createMarker(options: any) {
        return this._scriptPromise.then(() => {
            return Promise.resolve(
                new (<any>window).google.maps.Marker(options)
            );
        });
    }
        ...
        this._mapPromise.then(() => {
            ...
            for (let marker of this._renderedMarkers) {
                ...
                if (isNaN(lat) || isNaN(lng)) {
                    console.warn(`Marker returned NaN for lat/lng`, {
                        marker,
                        lat,
                        lng,
                    });
    ...
    renderPolygon(polygonObject: any = []) {
        ...
        if (Array.isArray(paths)) {
            paths = paths.map((x) => {
                return new (<any>window).google.maps.LatLng(
                    x.latitude,
                    x.longitude
                );
            });
    ...
    polygonsChanged(newValue: any) {
        ...
        this._mapPromise.then(() => {
            Promise.all(
                ...
                .then((polygons) => {
                    return Promise.all(
                        polygons.map(this.renderPolygon.bind(this))
                    );
                })
                .then(() => {
                    /**
