...
import { ITheme, Theme } from '../services/theme';
...
export class VideosOld {
    ...
    async created(params, config) {
        ...
        this.ea.subscribe('NEW-VIDEO', (msg: { new_video_rec: string }) => {
            this.add_video(msg.new_video_rec);
        });
        this.ea.subscribe('VIDEO-INFO-CHANGED', (msg: { changes: string }) => {
    ...
    add_video(new_video_rec) {
        ...
        const n = this.video_list.length;
        const r = n % this.videos_per_page;
    ...
    new_first_index(step) {
        const idx = this.first_index + step * this.videos_per_page;
    ...
    delete_video(video) {
        this.api
            .call_server('videos/delete_video', { video_id: video.id })
            .then(() => {
                const idx = this.video_list.findIndex((v) => v.id == video.id);
