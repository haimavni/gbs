...
export class Videos {
    ...
    constructor(
        ...
        this.order_options = [
            ...
            {
                name: i18n.tr("videos.recently-uploaded"),
    ...
    refresh_video_thumbnails() {
        ...
        this.api
            .call_server("videos/refresh_video_thumbnails", {
                selected_videos: selected_videos,
            })
            .then(() => {
                const video_list = this.video_list.filter((v) =>
                    this.selected_videos.has(v.id)
                );
    ...
    async view_video(video, event, member_id, keywords) {
        ...
        if (
            ...
            let url = this.misc.make_url(
                "annotate-video",
                `${video.id}/*?video_src=${video.src}&video_type=${video.video_type}&video_name=${video.name}&cuepoints_enabled=true&member_id=${member_id}&keywords=${keywords}`
            );
    ...
    handle_order_change(event) {
        ...
        switch (this.params.order_option.value) {
            case "by-name":
                this.video_list.sort((vid1, vid2) =>
                    vid1.name < vid2.name ? -1 : vid1.name > vid2.name ? +1 : 0
                );
                break;
            case "old-to-new":
                this.video_list.sort((vid1, vid2) =>
                    vid1.video_date_datestr < vid2.video_date_datestr
                        ? -1
                        : vid1.video_date_datestr > vid2.video_date_datestr
                        ? -1
                        : 0
            ...
            case "recently-uploaded":
                this.video_list.sort((vid1, vid2) =>
                    vid1.id < vid2.id ? +1 : vid1.id > vid2.id ? -1 : 0
                );
                break;
            default:
                break;
