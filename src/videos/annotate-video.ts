...
export class AnnotateVideo {
    ...
    constructor(
        ...
        private readonly ytKeeper: YtKeeper
    ) {
    ...
    attached() {
        this.track_segment = setInterval(
            () => this.detect_segment_change(),
            1000
        );
    }
    ...
    get_video_info(video_id, what = null) {
        return this.api
            ...
            .then((response) => {
                ...
                this.cue_points = cue_points.map(
                    (cp) =>
                        new CuePoint(
                            cp.cue_id,
                            cp.time,
                ...
                if (this.member_id) {
                    ...
                    for (let cue of this.cue_points) {
                        ...
                        n += 1;
                        if (
                            cue.member_ids.find((mid) => mid == this.member_id)
                        ) {
                            cue.cls = "hilited";
    ...
    add_cue_point() {
        ...
        this.api
            .call_server_post("videos/update_video_cue_points", {
                video_id: this.video_id,
                cue_points: this.cue_points,
            })
            .then((response) => {
                if (response.cue_id) cue.cue_id = response.cue_id;
            });
    }
    ...
    jump_to_cue(cue, auto = false) {
        ...
        if (!auto) this.player.currentTime = cue.time;
    ...
    select_members(cue) {
        ...
        this.dialog
            ...
            .whenClosed((response) => {
                ...
                cue.member_ids = Array.from((response.value as any).member_ids);
                this.api
                    .call_server_post("videos/update_cue_members", {
                        video_id: this.video_id,
                        cue_id: cue.cue_id,
                    ...
                    .then((response) => {
                        if (!cue.cue_id) cue.cue_id = response.cue_id;
                    });
            });
    }
