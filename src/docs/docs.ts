...
export class Docs {
    ...
    constructor(
        ...
        this.order_options = [
            ...
            {
                name: i18n.tr("docs.recently-uploaded"),
    ...
    activate(params, config) {
        ...
        if (this.doc_list && this.doc_list.length > 0 && !params.keywords)
            return;
        if (
            params.keywords == this.params.keywords_str &&
            this.doc_list &&
            this.doc_list.length > 0
        )
            return;
    ...
    update_doc_list() {
        ...
        return this.api
            .call_server_post("docs/get_doc_list", { params: this.params })
            .then((result) => {
    ...
    update_doc_segment_list() {
        ...
        this.params.editing = this.user.editing;
        return this.api
            .call_server_post("docs/get_doc_segment_list", {
                params: this.params,
            })
            .then((result) => {
                this.editing_filters = false;
                ...
                for (let doc_seg of this.doc_segment_list) {
                    doc_seg.title =
                        '<span dir="rtl">' + doc_seg.title + "</span>";
                    if (doc_seg.story) {
    ...
    apply_topics_to_checked_docs() {
        this.api
            .call_server_post("docs/apply_to_checked_docs", {
                params: this.params,
            })
            .then((response) => {
    ...
    update_list() {
        if (this.view_doc_segments) this.update_doc_segment_list();
    ...
    update_topic_list() {
        ...
        this.api
            .call_server_post("topics/get_topic_list", {
                params: this.params,
                usage: this.user.editing ? null : topic_type,
            })
            .then((response) => {
    ...
    toggle_selection(doc, event, index) {
        ...
        if (event.keys.altKey) {
            this.checked_docs = new Set();
            if (doc.story.checked) this.checked_docs.add(doc.story_id);
            for (let d of this.doc_list) {
                if (d.story_id != doc.story_id) d.story.checked = false;
            }
    ...
    delete_checked_docs() {
        this.api
            .call_server_post("docs/delete_checked_docs", {
                params: this.params,
            })
            .then((response) => {
                this.params.checked_doc_list = [];
    ...
    get phase() {
        ...
        this.options_settings.update({
            mergeable:
                result != "applying-to-docs" && result != "selecting-docs",
            ...
            hide_higher_options:
                this.checked_docs.size > 0 && this.user.editing,
    ...
    jump_to_the_full_doc(event, doc) {
        ...
        if (this.single_doc_entry) this.view_details(doc, event);
        else this.openDialog(doc);
    }
    ...
    handle_order_change(event) {
        switch (this.params.order_option.value) {
            case "by-name":
                this.doc_list.sort((doc1, doc2) =>
                    doc1.name < doc2.name ? -1 : doc1.name > doc2.name ? +1 : 0
                );
                break;
            case "old-to-new":
                this.doc_list.sort((doc1, doc2) =>
                    doc1.doc_date < doc2.doc_date
                        ? -1
                        : doc1.doc_date > doc2.doc_date
                        ? -1
                        : 0
            ...
            case "recently-uploaded":
                this.doc_list.sort((doc1, doc2) =>
                    doc1.id < doc2.id ? +1 : doc1.id > doc2.id ? -1 : 0
                );
                break;
            default:
                break;
