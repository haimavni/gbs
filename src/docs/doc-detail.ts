...
export class DocDetail {
    ...
    get_doc_info(doc_id) {
        this.api
            .call_server_post("docs/get_doc_info", {
                doc_id: doc_id,
                caller: this.caller,
            })
            .then((response) => {
    ...
    get_doc_segment_info(doc_segment_id) {
        ...
        this.api
            .call_server_post("docs/get_doc_segment_info", {
    ...
    page_num_selected(event) {
        ...
        this.api
            .call_server_post("docs/create_segment", {
                doc_id: this.doc_id,
                page_num: page_num,
            ...
            .then((response) => {
                ...
                this.doc_segments.splice(
                    this.doc_segments.length,
                    0,
                    doc_segment
                );
    ...
    update_doc_date(customEvent) {
        ...
        this.undo_list.push({
            what: "doc-date",
            doc_date: {
                doc_date_str: this.curr_info.doc_date_str,
                doc_date_datespan: this.curr_info.doc_date_datespan,
            },
        });
        this.curr_info.doc_date_str = this.doc_date_str.slice(0);
        this.curr_info.doc_date_datespan = this.doc_date_datespan;
        const story_id = this.curr_doc_segment
            ? this.curr_doc_segment.story_id
            : this.doc_story_id;
    ...
    handle_topic_change(event) {
        ...
        this.undo_list.push({
            what: "topics",
            photo_topics: this.curr_info.doc_topics,
        });
    ...
    init_selected_topics() {
        ...
        for (let opt of this.doc_topics) {
            ...
            let itm = {
                option: opt,
                first: i == 0,
                last: i == this.doc_topics.length - 1,
                group_number: i + 1,
            };
    ...
    undo() {
        ...
        switch (command.what) {
            case "topics":
                ...
                break;
            case "doc-date":
                this.curr_info.doc_date_str = this.doc_date_str =
                    command.doc_date.doc_date_str;
                this.doc_date_datespan = command.doc_date.doc_date_datespan;
                this.api.call_server_post("docs/update_doc_date", {
                    doc_date_str: this.doc_date_str,
                    doc_date_datespan: this.doc_date_datespan,
                    doc_id: this.doc_id,
                });
                break;
        }
    ...
    go_back() {
        if (this.caller == "docs")
            this.router.load(
                "docs"
            ); //strange bug causes it to go prev until the first
        else {
            this.router.back();
        }
    }
    ...
    get_doc_by_idx(idx) {
        ...
        if (this.full_doc) this.get_doc_info(did);
        else {
    ...
    select_doc_members() {
        ...
        this.dialog
            ...
            .whenClosed((response) => {
                ...
                member_ids = Array.from((response.value as any).member_ids);
                if (this.curr_doc_segment) {
                    this.api
                        .call_server_post("docs/update_doc_segment_members", {
                            doc_segment_id: this.curr_doc_segment.segment_id,
    ...
    get doc_src() {
        ...
        src += `?d=${this.counter}`;
        this.counter += 1;
        src += this.curr_doc_segment
            ? `#page=${this.curr_doc_segment.page_num}`
            : "";
        return src;
    }
    ...
    update_story_preview(event) {
        ...
        if (this.curr_doc_segment) {
            const doc_segment = this.doc_segments.find(
                (ds) => ds.segment_id == this.curr_doc_segment.segment_id
            );
            if (doc_segment) doc_segment.name = this.doc_name;
        }
        let story_id = this.doc_story.story_id;
        this.api.call_server_post("docs/update_story_preview", {
            story_id: story_id,
        });
    }
        ...
        this.dialog
            .open({
                ...
                model: {
                    multi: true,
                    back_to_text: "docs.back-to-doc",
                    preselected: doc_segment.member_ids,
                },
                lock: false,
                rejectOnCancel: false,
            })
            .whenClosed((response) => {
                this.theme.hide_title = false;
                if (response.status === 'cancel') return;
                doc_segment.member_ids = Array.from((response.value as any).member_ids);
    ...
    toggle_height() {
        if (this.expanded == "expanded") this.expanded = "";
    ...
    select_segment(doc_segment) {
        ...
        this.curr_doc_segment = this.doc_segments.find(
            (ds) => ds.segment_id == doc_segment.segment_id
        );
    ...
    remove_segment(doc_segment) {
        this.api
            .call_server("docs/remove_doc_segment", {
                doc_segment_id: doc_segment.segment_id,
            })
            .then((response) => {
                const idx = this.doc_segments.findIndex(
                    (ds) => ds.segment_id == doc_segment.segment_id
                );
    ...
    save_thumbnail(event: Event) {
        event.stopPropagation();
        this.working = true;
        this.api.uploadFiles(this.user.id, this.thumbnails, "DOC-SEG-THUMB", {
            doc_id: this.doc_id,
            segment_id: this.curr_doc_segment.segment_id,
