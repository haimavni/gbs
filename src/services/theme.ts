...
export const ITheme = DI.createInterface<ITheme>("ITheme", (x) =>
    x.singleton(Theme)
);
...
export class Theme {
    ...
    constructor(
        ...
        this.api.call_server("photos/get_theme_data").then((response) => {
    ...
    async set_locale() {
        ...
        try {
            ...
            //let userLangs = navigator.languages;
            let hasHebrew =
                userLangs && userLangs.find((lang) => lang.startsWith("he"));
    ...
    handle_content_resize() {
        ...
        if (changed) {
            this.eventAggregator.publish("WINDOW-RESIZED", {
                width: this.width,
                height: this.height,
            });
        }
    ...
    get_locale_overrides() {
        this.api
            .call_server("default/get_locale_overrides")
            .then((response) => {
    ...
    set_locale_override(key, value) {
        this.api
            .call_server_post("default/set_locale_override", {
                lang: this.i18n.getLocale(),
                key: key,
                value: value,
            })
            .then((response) => {
                this.get_locale_overrides();
            });
    }
    ...
    froala_key() {
        ...
        let lifestone_key =
            "qA2F1G1I2A2D3lA6F6D5G4A1D3C10A3A5F6gptxzsummaG4oinf==";
