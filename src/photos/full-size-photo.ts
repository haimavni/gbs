...
export class FullSizePhoto {
    ...
    constructor(
        ...
        this.share_on_facebook_txt = this.i18n.tr(
            "user.sharing.share-on-facebook"
        );
    ...
    get_faces(photo_id) {
        ...
        this.api
            .call_server("photos/get_faces", { photo_id: photo_id })
            .then((data) => {
    ...
    get_articles(photo_id) {
        this.articles = [];
        this.api
            .call_server("photos/get_articles", { photo_id: photo_id })
            .then((data) => {
                this.articles = data.articles;
                for (let article of this.articles) {
                    article.name =
                        '<span dir="rtl">' + article.name + "</span>";
    ...
    get_photo_info(photo_id) {
        this.api
            .call_server("photos/get_photo_info", { photo_id: photo_id })
            .then((data) => {
                this.photo_info.name = data.name;
                this.photo_info.photographer = data.photographer;
                this.photo_info.photographer_known = Boolean(
                    this.photo_info.photographer
                );
                if (!this.photo_info.photographer_known) {
                    this.photo_info.photographer = this.i18n.tr(
    ...
    assign_face_or_member(face) {
        if (face.article_id) this.assign_article(face);
        else this.assign_member(face);
    }
    ...
    assign_article(article) {
        this.dialogService
            ...
            .whenClosed((response) => {
                ...
                let mi =
                    response.value && (response.value as any).new_article
                        ? (response.value as any).new_article.article_info
                        : null;
                if (mi) {
                    article.name = article.article_id
                        ? mi.name
                        : mi.first_name + " " + mi.last_name;
                    article.article_id = (
                        response.value as any
                    ).new_article.article_info.id;
                    return;
                }
                article.article_id = (response.value as any).article_id;
                let make_profile_photo = (response.value as any)
                    .make_profile_photo;
                this.api
                    .call_server_post("photos/save_article", {
                        face: article,
    ...
    assign_member(face) {
        ...
        this.dialogService
            ...
            .whenClosed((response) => {
                ...
                let mi =
                    (response.value as any) &&
                    (response.value as any).new_member
                        ? (response.value as any).new_member.member_info
                        : null;
                if (mi) {
                    face.name = mi.first_name + " " + mi.last_name;
                    face.member_id = (
                        response.value as any
                    ).new_member.member_info.id;
                    return;
                }
                face.member_id = (response.value as any).member_id;
                let make_profile_photo = (response.value as any)
                    .make_profile_photo;
                this.api
                    .call_server_post("photos/save_face", {
                        face: face,
                        make_profile_photo: make_profile_photo,
                        old_member_id: old_member_id,
                    })
                    .then((response) => {
                        let idx = this.candidates.findIndex(
                            (m) => m.member_id == face.member_id
                        );
    ...
    get instruction() {
        if (this.marking_face_active) {
            ...
            if (this.marking_articles) s += "-article";
    ...
    private jump_to_member(member_id) {
        ...
        this.router.load("member-details", {
            parameters: { id: member_id, keywords: "" },
        });
    }
    ...
    private jump_to_article(article_id) {
        ...
        this.router.load("article-details", {
            parameters: { id: article_id, keywords: "" },
        });
    }
    ...
    mark_face(event) {
        ...
        if (this.marking_articles) this.articles.push(face);
    ...
    public dragstart(face, customEvent: CustomEvent, what) {
        ...
        let face_id =
            what == "face-" ? what + face.member_id : what + face.article_id;
    ...
    public dragmove(face, customEvent: CustomEvent) {
        ...
        customEvent.stopPropagation();
        let event = customEvent.detail;
        let id = face.article_id
            ? "article-" + face.article_id
            : "face-" + face.member_id;
    ...
    public save_photo_crop(event) {
        ...
        let photo_id =
            this.slide[this.slide.side].photo_id || this.slide.photo_id; //temporary bug hider
        this.api
            .call_server_post("photos/crop_photo", {
                photo_id: photo_id,
    ...
    public do_crop(customEvent: CustomEvent) {
        ...
        if (this.crop_sides == "sw" || this.crop_sides == "se") {
            ...
            this.crop_height = Math.min(
                this.crop_height,
                height - this.crop_top
            );
        }
    }
    ...
    rotate_photo(event) {
        ...
        let photo_id =
            this.slide[this.slide.side].photo_id || this.slide.photo_id; //temporary bug hider
        this.api
            .call_server("photos/rotate_selected_photos", {
                selected_photo_list: [photo_id],
    ...
    async share_on_facebook(event) {
        ...
        this.popup.popup(
            "SHARER",
            href,
            `height=600,width=526,left=${left},top=100`
        );
    }
    ...
    nobody(event) {
        ...
        this.api.call_server("photos/mark_as_recogized", {
            photo_id: this.slide[this.slide.side].photo_id,
    ...
    slide_idx() {
        ...
        return this.slide_list.findIndex(
            (slide) => slide.photo_id == this.slide.photo_id
        );
    }
    ...
    get_slide_by_idx_list_ids(idx) {
        ...
        this.api
            .call_server("photos/get_photo_detail", { photo_id: pid })
            .then((response) => {
    ...
    handle_context_menu(face, event) {
        ...
        this.dialogService
            ...
            .whenClosed((response) => {
                ...
                if (response.status !== "cancel") {
                    let command = (response.value as any).command;
                    if (command == "cancel-identification") {
                        ...
                        if (face.article_id) {
                            if (face.article_id > 0)
                                this.api.call_server_post(
                                    "photos/save_article",
                                    { face: face }
                                );
                            else this.assign_article(face);
                        } else {
                            if (face.member_id > 0)
    ...
    get face_moved() {
        ...
        let id = current_face.article_id
            ? "article-" + current_face.article_id
            : "face-" + current_face.member_id;
        let el = document.getElementById(id);
        if (!el) return;
        let face_location = this.face_location(current_face);
        el.style.left = face_location.left;
