...
export class Photos implements IRouteableComponent {
    ...
    loading(
        ...
        if (instruction.route.id == "associate-photos") {
            this.caller_id = params.caller_id as any;
            this.caller_type = params.caller_type;
            let arr;
            if (params.associated_photos) {
                arr = (params.associated_photos as any).map((i) => Number(i));
            } else {
                arr = [];
            }
            ...
            this.params.photo_ids = params.photo_ids as any[];
            this.photo_list = [];
        } else {
        ...
        this.dialog
            ...
            .whenClosed((response) => {
                ...
                this.params.selected_member_ids = Array.from(
                    (response.value as any).member_ids
                );
    ...
    pair_selected_photos() {
        this.api
            ...
            .then((response) => {
                ...
                let front_photo = this.photo_list.find(
                    (item) => item.photo_id == front_id
                );
                front_photo.flipable = "flipable";
                let back_photo = this.photo_list.find(
                ...
                let idx = this.photo_list.findIndex(
                    (item) => item.photo_id == back_id
                );
    ...
    photo_info_content(photo) {
        let photographer = this.photographer_list.find(
            (p) => p.id == photo.photographer_id
        );
        let photographer_name = photographer
            ? photographer.name
        ...
        this.api
            ...
            .then((result) => {
                ...
                for (let patch of photo_patches) {
                    if (!patch) continue;
                    let new_photo = this.photo_list.find(
                        (photo) => photo.photo_id == patch.photo_to_delete
                    );
                    let patch_target = this.photo_list.find(
                        (photo) => photo.photo_id == patch.photo_to_patch
                    );
                    let patch_target_idx = this.photo_list.findIndex(
                        (photo) => photo.photo_id == patch.photo_to_patch
                    );
                    if (patch_target) {
    ...
    get search_member_str() {
        if (this.with_a_member) return "photos.show-all-photos";
