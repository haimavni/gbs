...
export class StoryDetail {
    ...
    constructor(
        ...
        @IArticleList readonly article_list: IArticleList,
    ...
    attached() {
        ...
        this.subscriber = this.eventAggregator.subscribe(
            'Zoom2',
            (payload: {
                offset: string;
                ...
                this.misc.save(
                    ['story_slides_offset', this.story.story_id],
                    offset
                );
        ...
        this.subscriber1 = this.eventAggregator.subscribe(
            'STORY_WAS_SAVED',
            (payload: { story_data: { story_id: string } }) => {
                this.refresh_story(payload.story_data.story_id);
            }
        );
    }
    ...
    refresh_story(story_id) {
        if (this.new_story) {
            ...
            this.router.load(`/story-detail`, {
                parameters: {
                    id: story_id,
                    what: this.story_type,
                },
            });

            return;
        }
        const sid = this.story ? this.story.story_id : null;
    ...
    async loading(params) {
        ...
        const used_for = params.used_for;
        if (used_for) {
            this.used_for = parseInt(used_for);
        } else {
            this.used_for =
                params.what && params.what == 'term'
                    ? this.api.constants.story_type.STORY4TERM
                    : params.what && params.what == 'help'
                    ? this.api.constants.story_type.STORY4HELP
                    : this.api.constants.story_type.STORY4EVENT;
        }
        this.story_list = params.story_list || [];
        const story_id = params.id;
        this.story_idx = this.story_list.findIndex((itm) => itm == story_id);
        await this.update_topic_list();
        this.story_type =
            this.used_for == this.api.constants.story_type.STORY4TERM
                ? 'term'
                : this.used_for == this.api.constants.story_type.STORY4HELP
                ? 'help'
                : 'story';
        this.get_story_details(story_id);
        const offset = this.misc.load(['story_slides_offset', story_id]);
        if (offset) this.move_to = offset;
    }

    get_story_details(story_id) {
        const what =
            this.story_type == 'story'
                ? 'EVENT'
                : this.story_type == 'help'
                ? 'HELP'
                : 'TERM';
        this.api.getStoryDetail({ story_id: story_id }).then((response) => {
            ...
            if (this.story.story_id == 'new') {
                ...
                this.story.story_text = this.i18n.tr(
                    'stories.new-story-content'
                );
            ...
            const story_date = response.story_date;
            ...
            this.source = this.api.call_server_post(
                'members/get_story_photo_list',
                { story_id: story_id, story_type: this.story_type }
            );
            this.source.then(
                (response) =>
                    (this.has_associated_photos =
                        response.photo_list.length > 0)
            );
        });
    }
    ...
    update_associated_members() {
        ...
        const member_ids = this.members.map((member) => Number(member.id));
        this.dialog
            .open({
                component: () => MemberPicker,
                model: {
                    multi: true,
                    back_to_text: 'members.back-to-story',
            ...
            .whenClosed((response: any) => {
                ...
                const member_ids = Array.from(response.output.member_ids);
                const member_set = new Set(member_ids);
    ...
    update_associated_articles() {
        this.theme.hide_title = true;
        const article_ids = this.articles.map((article) => Number(article.id));
        this.dialog
            .open({
            ...
            .whenClosed((response: any) => {
                this.theme.hide_title = false;
                if (response.status === 'cancel') return;
                const article_ids = Array.from(response.output.article_ids);
                const article_set = new Set(article_ids);
                this.api.call_server_post('articles/save_group_articles', {
                    caller_id: this.story.story_id,
                    caller_type: this.story_type,
                    article_ids: article_ids,
                });
                this.articles = this.article_list.articles.article_list.filter(
                    (article) => article_set.has(article.id)
                );
            });
    }
    ...
    go_prev(event) {
        ...
        if (this.story_idx > 0) {
            this.story_changed = true;
            this.story_idx -= 1;
            const story_id = this.story_list[this.story_idx];
    ...
    accept_candidate(candidate_id, idx) {
        const mems = this.candidates.slice(idx, idx + 1);
        this.candidates.splice(idx, 1);
        const mem = mems[0];
        this.members.push(mem);
        this.api.call_server_post('members/add_story_member', {
            story_id: this.story.story_id,
    ...
    accept_article_candidate(candidate_id, idx) {
        const arts = this.article_candidates.slice(idx, idx + 1);
        this.article_candidates.splice(idx, 1);
        const art = arts[0];
        this.articles.push(art);
        this.api.call_server_post('members/add_story_article', {
            story_id: this.story.story_id,
            candidate_id: candidate_id,
        });
    }
    ...
    keep_only_digits(event, idx) {
        ...
        const m = key.match(/[0-9/]/) || key == 'Backspace' || key == 'Delete';
        if (!m) {
    ...
    sorting_key_changed(event, idx) {
        this.undo_list.push({
            what: 'sorting-key',
            sorting_key: this.curr_info.sorting_key,
        });
        this.curr_info.sorting_key = this.sorting_key.slice(0);
        this.api.call_server_post('members/save_sorting_key', {
            story_id: this.story.story_id,
            sorting_key: this.sorting_key,
        });
    }

    update_topic_list() {
        this.api
            .call_server_post('topics/get_topic_list', { usage: 'ET' })
            .then((result) => {
    ...
    init_selected_topics() {
        ...
        let i = 0;
        const story_topics = this.story_topics || [];
        for (const opt of story_topics) {
            opt.sign = '';
            const itm = {
                option: opt,
                first: i == 0,
                last: i == this.story_topics.length - 1,
                group_number: i + 1,
            };
    ...
    handle_topic_change(event) {
        ...
        this.undo_list.push({
            what: 'topics',
            story_topics: this.curr_info.story_topics,
        });
    ...
    update_story_date(customEvent) {
        ...
        this.api.call_server_post('members/update_story_date', {
            story_date_str: event.date_str,
            story_date_datespan: event.date_span,
            story_id: this.story.story_id,
        });
    }
    ...
    undo() {
        ...
        switch (command.what) {
            ...
            case 'story-date':
                ...
                this.curr_info.story_date_str =
                    command.story_date.story_date_str;
                this.story_date_datespan =
                    command.story_date.story_date_datespan;
                this.curr_info.story_date_datespan =
                    command.story_date.story_date_datespan;
                this.api.call_server_post('members/update_story_date', {
    ...
    remove_story_from_book() {
        this.api
            .call_server_post('topics/remove_story_from_book', {
                story_id: this.story.story_id,
