...
export class MemberDetail {
    ...
    refresh_story(data) {
        ...
        let story = this.member_stories.lst.find(
            ...
            .whenClosed((response) => {
                if (
                    ...
                    this.api.call_server_post("members/set_member_story_id", {
                        ...
                        story_id: (response.value as any).story_id,
                    });
                }
            });
    }
    ...
    _set_heights() {
        ...
        let no_member_stories = this.member
    ...
    divorce(mem_id, who) {
        //event.stopPropagation();
        if (!this.user.editing || !this.user.privileges.ADMIN) return;
        document.body.classList.add("black-overlay");
        this.dialog
            .open({
                component: () => Divorce,
            ...
            .whenClosed((response) => {
                const spouse_id = (response.value as any).spouse_id;
                const what = (response.value as any).what;
                this.api
                    .call_server("members/divorce", {
                        member_id: this.member_id,
                        spouse_id: spouse_id,
                        hide_spouse: (response.value as any).hide_spouse,
                        what: what,
                    })
                    .then((response) => {
                        this.member.family_connections.spouses =
                            response.spouses;
                    });
                document.body.classList.remove("black-overlay");
            });
