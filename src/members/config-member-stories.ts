...
export class ConfigMemberStories implements IRouteableComponent {
    ...
    loading(model) {
        ...
        for (const t of this.story_type_arr) {
    ...
    get_story_types() {
        ...
        for (const st of this.all_member_stories) {
            if (st.used_for != 1) result.add(st.used_for);
        }
        return result;
    }
    ...
    update_story_list() {
        this.member_stories.lst = this.all_member_stories.filter(
            (item) => this.story_types.has(item.used_for) || item.used_for == 1
        );
        if (this.filter) {
            this.member_stories.lst = this.member_stories.lst.filter(
                (item) =>
                    item.name.includes(this.filter) ||
                    item.used_for == 1 ||
                    item.story_text.includes(this.filter)
            );
        }
