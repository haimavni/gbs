...
import { IPopup, Popup } from "../services/popups";
...
export class UploadPhoto {
    ...
    attached() {
        this.subscriber = this.ea.subscribe("GROUP-PHOTO-UPLOADED", (msg: any) => {
            this.photos = [];
    ...
    get phase() {
        ...
        if (this.status_record.photo_uploaded) this.working = false;
    ...
    view_gallery() {
        ...
        let url = this.misc.make_url(
            "photos",
            `*?user_id=${this.status_record.user_id}&key=${key}`
        );
        this.popup.popup(
            "GALLERY",
            url,
        ...
        this.dialog
            .open({
                ...
                model: {
                    ...
                    hide_details_icon: !(
                        this.user.editing || slide.has_story_text
                    ),
                    list_of_ids: true,
                },
                lock: false,
